...
pub fn sell(ctx:Context<ASell>, base_amount: u64, min_quote_amount: u64) -> Result<()> {
    ...
    require!(output_amount >= min_quote_amount, PumpFunError::TooLowOuputSol);
...
pub struct ASell<'info> {
    ...
    pub seller_base_ata: Box<Account<'info, TokenAccount>>, // Seller's Token ATA
    #[account(
        init_if_needed,
        payer = seller,
        associated_token::mint = quote_mint,
    ...
    pub seller_quote_ata: Box<Account<'info, TokenAccount>>, // Seller's WSOL ATA
