use crate::{
    ...
    CreateEvent,
    error::PumpFunError,
    utils::{check_balance, sync_native_amount},
};
...
pub fn create_pool(ctx: Context<ACreatePool>) -> Result<()> {
    ...
    let creator_base_ata = &ctx.accounts.creator_base_ata; // Creator's Token ATA
    let token_program = ctx.accounts.token_program.to_account_info();
    
    // Initialize all members of pool_state
    pool_state.owner = creator.key(); // Creator's address
    pool_state.base_mint = creator_base_ata.mint; // Token mint address
    pool_state.quote_mint = ctx.accounts.quote_mint.key(); // WSOL address
    pool_state.real_base_reserves = main_state.total_supply; // Total supply of tokens is all put into the pool
    ...
    pool_state.virt_quote_reserves = main_state.init_virt_quote_reserves; // Initial virtual SOL reserves
    pool_state.real_quote_threshold = main_state.real_quote_threshold; // Real SOL threshold
...
pub struct ACreatePool<'info> {
    ...
    pub base_mint: Box<Account<'info, Mint>>, // Token mint address
    #[account(
        constraint = quote_mint.key().to_string() == NATIVE_MINT_STR @ PumpFunError::UnknownToken
    )]
    pub quote_mint: Box<Account<'info, Mint>>, // WSOL address
    ...
    pub creator_base_ata: Box<Account<'info, TokenAccount>>, // Creator's Token ATA
    ...
    #[account(
        ...
        constraint = reserver_base_ata.amount == 0 // PoolState's token amount must be 0
    )]
