...
pub fn buy_tokens_from_exact_sol(ctx:Context<ABuy>, quote_amount: u64, min_base_amount: u64) -> Result<()> {
    ...
    if (pool_state.real_quote_reserves + (_quote_amount - fee) > pool_state.real_quote_threshold) {
        let limit_quote_amount = calculate_total_amount(main_state.trading_fee, pool_state.real_quote_threshold - pool_state.real_quote_reserves);
        if (_quote_amount > limit_quote_amount) {
            _quote_amount = limit_quote_amount;
...
pub fn buy_exact_tokens_from_sol(ctx:Context<ABuy>, base_amount: u64, max_quote_amount: u64) -> Result<()> {
    ...
    require!(total_quote_amount <= max_quote_amount, PumpFunError::TooMuchInputSol);
...
pub struct ABuy<'info> {
    ...
    pub fee_quote_ata: Box<Account<'info, TokenAccount>>, // FeeRecipient's WSOL ATA
    ...
    pub buyer_base_ata: Box<Account<'info, TokenAccount>>, // Buyer's Token ATA
    #[account(
        init_if_needed,
        payer = buyer,
        associated_token::mint = quote_mint,
    ...
    pub buyer_quote_ata: Box<Account<'info, TokenAccount>>, // Buyer's WSOL ATA
